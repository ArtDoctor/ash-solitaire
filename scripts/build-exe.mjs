...
async function main() {
  ...
  const shellIcon = join(root, "src-tauri", "icons", "icon.ico");

  // Tauri embeds the app icon at resource id 32512; Explorer prefers the "default" shell icon
  // (rcedit rewrites icon resources so File Explorer shows the same artwork as the taskbar).
  if (platform() === "win32" && existsSync(appExe) && existsSync(shellIcon)) {
    try {
      await rcedit(appExe, { icon: shellIcon });
      console.log("\nApplied shell/Explorer icon (rcedit) on solitaire.exe.\n");
    } catch (err) {
      console.error("\nWarning: rcedit could not set the .exe icon:", err);
    }
  }
